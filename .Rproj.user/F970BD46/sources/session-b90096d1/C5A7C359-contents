...
for(n_ind in seq_along(N.arr)){
  ...
  for(t_ind in seq_along(TT.arr)){
    ...
    total_err_mat = foreach(mc_iter = 1:R.max, .combine = rbind) %dopar% {
      ...
      F_est_trend <- array(dim = dim(F_est))
      F_for_trend <- matrix(nrow = 3, ncol = r)
      
      for(j in 1:r){
        result <- fit_smoothing_spline(F_est[,j], block_size = ceiling(sqrt(TT)))
        F_est_trend[,j] <- result$fit$y
        F_for_trend[,j] <- predict(result$fit, TT + c(1:3))$y
      }
      
      # # De-trend the estimated factors
      F_est_noise <- F_est - F_est_trend
      
      ### Task 1: Fit a VAR model
      ar_fit <- ar(F_est_noise, aic = TRUE, order.max = 5, demean = FALSE)
      F_for_var1 <- predict(ar_fit, n.ahead = 3)$pred
      
      ### Forecast the common components
      
      C_hat1 <- L_est[c(1:32), ] %*% t(F_for_var1 + F_for_trend)
      Y_ms <- Y
      Y_ms[W == 0] <- NA
      rownames(Y_ms) <- NULL
      colnames(Y_ms) <- NULL
      
      ## Only forecast for the first 32 users for N > 32
      ## Change this if you want more users to consider in the average error
      C_hat2 <- t(
        vapply(
          1:32,
      ...
      # Forecast error
      av_err1 = colMeans((C_hat1 - C0[c(1:32), ])^2) #Focus
      av_err2 = colMeans((C_hat2 - C0[c(1:32), ])^2)  #mSSA
...
out_dgp1 <- list(rep_focus_h1=rep_focus_h1,
...
save(out_dgp1, file = "data_files/DGP1/DGP1_out.RData")
