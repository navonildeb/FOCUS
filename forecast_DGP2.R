...
for(n_ind in seq_along(N.arr)){
  ...
  for(t_ind in seq_along(TT.arr)){
    ...
    total_err_mat = foreach(mc_iter = 1:R.max, .combine = rbind) %dopar% {
      ...
      # Forecast error
      av_err1 = colMeans((C_hat1 - C0[1:32, ])^2) #Focus
      av_err2 = colMeans((C_hat2 - C0[1:32, ])^2)  #mSSA
...
out_dgp2 <- list(rep_focus_h1=rep_focus_h1,
...
save(out_dgp2, file = "data_files/DGP2/DGP2_out.RData")
