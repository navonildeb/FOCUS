  ...
  for(n_ind in seq_along(N.arr)){
    ...
    for(t_ind in seq_along(TT.arr)){
      ...
      total_arr = foreach(mc_iter = 1:R.max) %dopar% {
        ...
        F0 <- gen.var1(A0, n.time = 1.5*TT, se.var)
        
        # Target of forecast 1,2,3-step ahead
        C0 <- cbind(L0 %*% (A0 %*%F0[TT,]), 
                    L0 %*% (A0^2 %*%F0[TT,]), 
                    L0 %*% (A0^3 %*%F0[TT,]))
        ...
        if(obs_pattern == "mcar"){
          ...
          file_name_W <- sprintf("data_files/DGP0/W_files/DGP0_simultW_N%d_T%d_iter%d.csv", N, TT, mc_iter)
          write.csv(W, file = file_name_W, row.names = FALSE)
        }
      }
      registerDoSEQ()
...
se.idio = 0.5 # Idiosyncratic error
...
generate_data(N.arr, TT.arr, R.max, r = 1, A1, A2, A3, B0, se.idio, se.loading, se.var, p_mcar, obs_pattern)
