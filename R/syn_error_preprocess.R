...
dgp_ind = 2 # change dgp_ind = 0, 1, 2 accordingly
TT.arr <- 2^c(5:8)
...
rep_syn_h1 <- array(dim = c(length(TT.arr), R.max))
rep_syn_h2 <- array(dim = c(length(TT.arr), R.max))
rep_syn_h3 <- array(dim = c(length(TT.arr), R.max))
timerep_syn <- array(dim = c(length(TT.arr), R.max))
...
for(t_ind in seq_along(TT.arr)){
  ...
  timerep_syn[t_ind, ] <- read.csv(sprintf("data_files/DGP%d/synout_files/N%d_T%d/synruntime_DGP%d_Y_N%d_T%d.csv",
                   dgp_ind, N, TT, dgp_ind, N, TT))[[1]]
}

syn_errors_final <- list(rep_syn_h1 = rep_syn_h1,
                  rep_syn_h2 = rep_syn_h2,
                  rep_syn_h3 = rep_syn_h3,
                  timerep_syn = timerep_syn)
