...
for(h in c(1:3)){
  ...
  err_focus <- switch(as.character(h),
                      "1" = out_dgp$rep_focus_h1[n_ind, seq_along(TT.arr), ],
                      "2" = out_dgp$rep_focus_h2[n_ind, seq_along(TT.arr), ],
                      "3" = out_dgp$rep_focus_h3[n_ind, seq_along(TT.arr), ]
  )
  
  err_ms <- switch(as.character(h),
                   "1" = out_dgp$rep_ms_h1[n_ind, seq_along(TT.arr), ],
                   "2" = out_dgp$rep_ms_h2[n_ind, seq_along(TT.arr), ],
                   "3" = out_dgp$rep_ms_h3[n_ind, seq_along(TT.arr), ]
  )
  ...
  #--- Summary stats for plotting (mean ± sd)
  df_summary <- df_all %>%
  ...
  plt <- ggplot(df_summary, aes(x = logT, y = mean_logerr, color = method, shape = method)) +
    ...
    scale_x_continuous(
      breaks = log2(c(32, 64, 128, 256, 512)),
      labels = as.expression(c(expression(2^5), expression(2^6), expression(2^7), expression(2^8), expression(2^9)))
    ) +
    scale_y_continuous(
      breaks = y_breaks,
    ...
    labs(
      ...
      title = sprintf("DGP-%d, h = %d", dgp_ind, h)
    ) +
  ...
  plt_name <- sprintf("figures/error_plots_vs_T/DGP%d_h%d.pdf", dgp_ind, h)
